...
def login():
	if request.method=='POST':
		session['username'] = request.form.get('username')  # Make session = whatever user entered in form
		session['email'] = request.form.get('email')
		return redirect(url_for('index'))   # now it goes ot the index function above
...
def submitSurvey():
	...
	if 'username' in session:
		...
		return render_template('results.html', name=username, email=email, surveyResponse=surveyResponse) # survey response is a dictionary
